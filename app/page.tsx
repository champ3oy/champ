      ...
      <div className='w-full h-full background-gradient'>
        ...
        <div className='mt-20 overflow-hidden'>
          <div className='flex w-full items-center justify-center'>
            {/* First set of divs */}
            {apps?.map((app, index) => (
              <Link href={app.link} target='_blank' key={`first-${index}`} className='flex-shrink-0 mx-5 flex flex-col items-center justify-center'>
                <img src={app.logo} alt={app.name} className='min-w-52 h-52 object-cover rounded-3xl ' />
                <p className='text-sm runde-medium text-black/70'>{app.name}</p>
              </Link>
            ))}
            {/* Duplicate set for seamless loop */}
            {/* {apps?.map((app, index) => (
              <div key={`second-${index}`} className='flex-shrink-0 mx-5 flex flex-col items-center justify-center'>
                <img src={app.logo} alt={app.name} className='min-w-52 h-52 object-cover rounded-3xl' />
                <p className='text-sm runde-medium text-black/70'>{app.name}</p>
              </div>
            ))} */}
